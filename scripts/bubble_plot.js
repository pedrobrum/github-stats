...
d3.csv(arquivo, function(error, data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    num_repos = parseInt(data[i]['repos']);
    num_issues = parseInt(data[i]['num_issues']);
    num_pulls = parseInt(data[i]['pull_requests']);

    pos = repos_lang.map(function(e) { return e.language; }).indexOf(lang);
    if (pos >= 0) {
      repos_lang[pos].repos += num_repos;
      repos_lang[pos].num_issues += num_issues;
      repos_lang[pos].nunm_pulls += num_pulls;
    }
    else
      repos_lang.push({
        language: lang,
  ...
  for(var i = 0; i < repos_lang.length; i++) {
    repos_lang[i].issues = repos_lang[i].num_issues/repos_lang[i].repos;
    repos_lang[i].pulls = repos_lang[i].num_pulls/repos_lang[i].repos;
  }
  ...
  var yScale = d3.scale.linear()
  	.range([height,0])
    // .domain(d3.extent(data, function(d) { return d.issues; }))
  	.domain(d3.extent(data, function(d) { return d.issues; }))
  	.nice();
  ...
  var rScale = d3.scale.sqrt()
  			...
  			.domain(d3.extent(data, function(d) { return d.repos; }));
  ...
  var voronoi = d3.geom.voronoi()
  	.x(function(d) { return xScale(d.pulls); })
  ...
  clipWrapper.selectAll(".clip")
  	...
  	.enter().append("clipPath")
    	.attr("class", "clip")
    	.attr("id", function(d) { return "clip-" + d.point.language; })
    	.append("path")
  ...
  // 	.data(data.sort(function(a,b) { return b.repos > a.repos; }))
  // 	.enter().append("circle")
  // 	.attr("class", function(d,i) { return "circle-wrapper " + d.language; })
  // 	.attr("clip-path", function(d) { return "url(#clip-" + d.language + ")"; })
  //     .style("clip-path", function(d) { return "url(#clip-" + d.language + ")"; })
  // 	.attr("cx", function(d) {return xScale(d.pulls);})
  // 	.attr("cy", function(d) {return yScale(d.issues);})
  ...
  // circleGroup.selectAll("data")
  // 	.data(data.sort(function(a,b) { return b.repos > a.repos; })) //Sort so the biggest circles are below
  // 	.enter().append("circle")
  // 		.attr("class", function(d,i) { return d.language; })
  // 		.attr("cx", function(d) {return xScale(d.pulls);})
  // 		.attr("cy", function(d) {return yScale(d.issues);})
  // 		.attr("r", function(d) {return rScale(d.repos);})
  ...
  // 	var element = d3.selectAll(".data."+d.language);
  //
  ...
  // 		html : true,
  // 		content: function() {
  // 			return "<span style='font-size: 11px; text-align: center;'>" + d.language + "</span>"; }
  // 	});
