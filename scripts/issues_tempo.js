  ...
  function issuesPorLinguagem() {
  	...
  	d3.csv(arquivo, function(error, data) {
  		...
  		var yhist = d3.scale.linear()
  							.domain([0, d3.max(data, function(x) { return x.issues; })])
  							.range([h, 0]);
  		...
  		var guide = d3.svg.line()
  					...
  					.y(function(x) { return yhist(x.issues) });
  		...
  		svg.selectAll("dot")
  			...
  			.attr("cy", function(x) { return yhist(x.issues); })
  			.on("mouseover", function(x) {
  							...
  							div.html("<b>" + (lang) + ' - ' + (x.year)
  									...
  									+ "Número de issues: " + x.num_issues.toLocaleString('pt-BR') + "<br/>"
  									+ "Issues por repositório: " + (x.issues).toFixed(2))
  		...
  		svg.append("g")
  			...
  			.style("font-size", "20px")
  			.text("Nº de issues por repositório");
  	});
  }
